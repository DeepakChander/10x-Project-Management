...
2. **SECOND:** Ask user for Archon host URL (default: `http://localhost:3737/`)
...
**First-time use:** You will be prompted for the Archon server URL (e.g., `http://localhost:3737/`). This will be remembered for the rest of the conversation.
...
Please provide the full URL (e.g., http://localhost:3737/ or http://192.168.1.100:3737):"
```

Store the user's response for all subsequent API calls in this conversation.

**Default if user is unsure:** `http://localhost:3737/`
...
python3 list_knowledge.py http://localhost:3737/
```
...
archon_host = "http://localhost:3737/"  # Use the URL provided by user
...
archon_host = "http://192.168.1.100:3737"  # Example
client = ArchonClient(base_url=archon_host)
```
...
archon_host = "http://localhost:3737/"  # Use user's actual host
...
archon_host = "http://localhost:3737/"  # Replace with user's actual host
...
archon_host = "http://localhost:3737/"
client = ArchonClient(base_url=archon_host)
...
```bash
cd .claude/skills/archon/scripts
python3 list_knowledge.py                      # Uses default localhost:3737
python3 list_knowledge.py http://192.168.1.100:3737  # Custom host
```
...
**Host URL:** Provided by user at skill activation (e.g., `http://localhost:3737/`, `http://192.168.1.100:3737`)
...
```bash
ARCHON_SERVER_PORT=3737  # API server port
