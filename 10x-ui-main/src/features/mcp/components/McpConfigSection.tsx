...
export const McpConfigSection: React.FC<McpConfigSectionProps> = ({ config, status, className }) => {
  ...
  const handleCursorOneClick = () => {
    ...
    showToast("Opening Cursor with 10x MCP configuration...", "info");
  };
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <div className={cn("p-3 rounded-lg", glassmorphism.background.blue, glassmorphism.border.blue)}>
        <p className="text-sm text-[#5B8C5A] dark:text-[#7AAC79]">
          <span className="font-semibold">Note:</span> 10x works with any application that supports MCP. Below are
      ...
      >
        ...
        <TabsContent value={selectedIDE} className="mt-6 space-y-4">
              ...
              {selectedConfig.steps.map((step) => {
                ...
                if (selectedIDE === "codex" && step.includes("npm install -g mcp-remote")) {
                  ...
                  return (
                    <li key={step}>
                      {parts[0]}
                      <code className="font-mono font-semibold bg-zinc-800 text-[#C0745F] px-1.5 py-0.5 rounded">
          ...
          {selectedIDE === "claudecode" && (
            ...
            >
              <code className="text-sm font-mono text-[#C0745F] dark:text-[#D4917A]">
