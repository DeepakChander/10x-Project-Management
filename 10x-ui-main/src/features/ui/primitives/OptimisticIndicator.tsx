...
export function OptimisticIndicator({
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      {showSpinner && <Loader2 className="h-3 w-3 animate-spin text-[#C0745F]/70" />}
      {pulseAnimation && <span className="text-xs text-[#C0745F]/50 animate-pulse">Saving...</span>}
    </div>
  );
}
...
export function withOptimisticStyles<T extends { className?: string }>(
  ...
  return (props: T) => (
    <Component
      {...props}
      className={cn(props.className, isOptimistic && "opacity-70 animate-pulse ring-1 ring-[#C0745F]/20")}
    />
  );
}
