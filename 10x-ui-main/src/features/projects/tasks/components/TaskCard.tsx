...
import type { Assignee, Task, TaskDependencyWithTitle, TaskPriority } from "../types";
...
export interface TaskCardProps {
  ...
  blockedBy?: TaskDependencyWithTitle[];
}
...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  // Filter to unresolved blockers (not done)
  const unresolvedBlockers = blockedBy?.filter((dep) => dep.depends_on_status !== "done") ?? [];
    ...
    >
      <Card
        ...
        className={cn(
          ...
          isHighlighted && "border-[#C0745F]/50 shadow-sm",
          isSelected && "border-[#C0745F] shadow-sm",
          "group-hover:border-[#C0745F]/70 dark:group-hover:border-[#C0745F]/50 group-hover:shadow-sm",
          optimistic && "opacity-80 ring-1 ring-[#C0745F]/30",
          isBlocked && "border-red-500/30 dark:border-red-400/20",
        )}
      >
            ...
            {isBlocked && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-red-500/10 text-red-500 dark:text-red-400 text-xs font-medium">
                ...
                <TooltipContent side="top" className="max-w-xs">
                  ...
                  <ul className="text-xs space-y-0.5">
                    {unresolvedBlockers.map((dep) => (
                      <li key={dep.id}>- {dep.depends_on_title}</li>
                    ))}
                  </ul>
                </TooltipContent>
              </Tooltip>
