...
export const taskService = {
  ...
  /**
   * Get all dependencies for a project (used by board/table views)
   */
  async getDependenciesByProject(projectId: string): Promise<TaskDependencyMap> {
    try {
      return await callAPIWithETag<TaskDependencyMap>(`/api/projects/${projectId}/dependencies`);
    } catch (error) {
  ...
  async getTaskDependencies(
    taskId: string,
  ): Promise<{ blocks: TaskDependencyWithTitle[]; blocked_by: TaskDependencyWithTitle[] }> {
    try {
  ...
  /**
   * Add a blocking dependency: taskId is blocked by dependsOnId
   */
  async addDependency(taskId: string, dependsOnId: string): Promise<TaskDependency> {
    try {
      const response = await callAPIWithETag<{ dependency: TaskDependency }>(
  ...
  async removeDependency(dependencyId: string): Promise<void> {
    try {
      await callAPIWithETag(`/api/dependencies/${dependencyId}`, { method: "DELETE" });
    } catch (error) {
