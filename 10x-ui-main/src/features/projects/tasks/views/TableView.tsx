import { Check, Edit, Lock, Tag, Trash2 } from "lucide-react";
...
import type { Assignee, Task, TaskDependencyMap, TaskDependencyWithTitle } from "../types";
...
const rowVariants = {
  ...
  hover:
    "hover:bg-gradient-to-r hover:from-[#C0745F]/5 hover:to-[#C0745F]/10 dark:hover:from-[#C0745F]/10 dark:hover:to-[#C0745F]/5",
  ...
  return (
    <tr
      ...
      className={cn(
        ...
        isOver && "bg-[#C0745F]/10 dark:bg-[#C0745F]/10 border-[#C0745F]",
      )}
    >
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-0.5 text-red-500 dark:text-red-400 shrink-0">
                    <Lock className="w-3 h-3" />
                    <span className="text-xs font-medium">{unresolvedBlockers.length}</span>
                  </div>
                </TooltipTrigger>
              ...
              <React.Fragment key={status}>
                ...
                {statusTasks.map((task, index) => (
                  <DraggableRow
                    ...
                    blockedBy={dependencyMap?.[task.id]?.blocked_by}
                  />
                ))}
              </React.Fragment>
            );
