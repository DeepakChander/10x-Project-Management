...
import type { CreateTaskRequest, Task, TaskDependencyMap, UpdateTaskRequest } from "../types";
...
// Fetch all dependencies for a project (used by board/table views)
export function useProjectDependencies(projectId: string | undefined) {
  return useQuery<TaskDependencyMap>({
    queryKey: projectId ? taskKeys.dependencies(projectId) : DISABLED_QUERY_KEY,
    queryFn: () => (projectId ? taskService.getDependenciesByProject(projectId) : Promise.reject("No project ID")),
    enabled: !!projectId,
    staleTime: STALE_TIMES.normal, // 30s - dependencies change infrequently
  });
}
...
export function useAddDependency(projectId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: taskKeys.dependencies(projectId) });
...
export function useRemoveDependency(projectId: string) {
  ...
  return useMutation({
    mutationFn: (dependencyId: string) => taskService.removeDependency(dependencyId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: taskKeys.dependencies(projectId) });
