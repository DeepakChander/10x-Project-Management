  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="flex-shrink-0">
            {selectedItem.type === "document" ? (
              <FileText className="w-5 h-5 text-[#C0745F]" />
            ) : (
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-6 scrollbar-thin">
        {selectedItem.type === "document" ? (
          <div className="prose prose-invert prose-sm max-w-none prose-headings:text-[#C0745F] prose-a:text-[#C0745F] prose-code:text-[#C0745F] prose-strong:text-white prose-pre:bg-muted prose-pre:border prose-pre:border-border">
            <ReactMarkdown
              components={{
                ...
                code: ({ children }) => <code className="px-1.5 py-0.5 rounded bg-muted">{children}</code>,
              }}
            >
          ...
          (() => {
            ...
            return (
              <div className="relative">
                <pre
                  className={[
                    "bg-muted dark:bg-muted border border-[#C0745F]/10 rounded-lg p-4",
        ...
        <div className="px-4 py-3 flex items-center justify-between text-xs text-gray-500">
          <div className="flex items-center gap-4">
            {selectedItem.metadata?.relevance_score != null && (
              <span>
                Relevance:{" "}
                <span className="text-[#C0745F]">{(selectedItem.metadata.relevance_score * 100).toFixed(0)}%</span>
              </span>
            )}
            {selectedItem.type === "document" &&
              selectedItem.metadata &&
              ...
              selectedItem.metadata.url && (
                <a
                  ...
                  className="text-[#C0745F] hover:text-[#A85A45] transition-colors underline"
                >
                  View Source
                </a>
              )}
