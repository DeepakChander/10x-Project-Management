...
const CompatibilityBadge: React.FC<CompatibilityBadgeProps> = ({ level, className = '' }) => {
  const badgeConfig = {
    full: { color: 'bg-green-500', text: '10x Ready', icon: '✓' },
  ...
  return (
    <div 
      className={`relative bg-gray-800/50 rounded-xl p-4 border-2 transition-all duration-300 cursor-pointer hover:shadow-lg hover:scale-[1.02] ${
        isSelected ? `${getCardBorderColor()} ring-2 ring-blue-400 shadow-[0_0_20px_rgba(192,116,95,0.3)]` : `${getCardBorderColor()} hover:border-gray-600 hover:bg-gray-800/70`
      }`}
      onClick={onSelect}
    >
      {/* Top-right badges */}
      ...
      <div className="border-t border-gray-600 pt-3">
        <div className="flex flex-wrap gap-4 text-xs">
          ...
          {model.architecture && (
            <div className="flex items-center">
              <span className="w-3 h-3 text-[#C0745F] mr-1">🏗️</span>
              <span className="text-gray-300">Arch: </span>
              <span className="text-[#C0745F] ml-1 capitalize">{model.architecture}</span>
            </div>
          )}
          ...
          {(model.format || model.parameters?.format) && (
            <div className="flex items-center">
              <span className="w-3 h-3 text-[#C0745F] mr-1">📦</span>
              <span className="text-gray-300">Format: </span>
              <span className="text-[#C0745F] ml-1 uppercase">{model.format || model.parameters?.format}</span>
            </div>
          )}
      ...
      if (response.ok) {
        ...
        const getArchonCompatibility = (model: any, modelType: string): 'full' | 'partial' | 'limited' => {
          if (modelType === 'chat') {
            ...
            // Well-tested models with full 10x support
            ...
            // Standard dimensions with excellent 10x support
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }} onClick={onClose}>
      <div className="bg-gray-900/95 border border-gray-800 rounded-xl w-full max-w-7xl h-[90vh] flex flex-col overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-green-500 via-blue-500 via-orange-500 to-[#C0745F] shadow-sm"></div>
        ...
        <div className="p-6 border-b border-gray-700">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-300">10x Compatibility:</span>
            <div className="flex gap-2">
              <Button
